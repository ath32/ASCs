...
source_total <- "/Users/atho/Documents/Project_ASCs/4_Outputs/CSVs/6.1_Thirdstops_all.csv"
data_total = read.csv(source_total, header = TRUE)

source_HEGs <- "/Users/atho/Documents/Project_ASCs/4_Outputs/CSVs/6.1_Thirdstops_HEGs.csv"
data_HEGs = read.csv(source_HEGs, header = TRUE)

source_LEGs <- "/Users/atho/Documents/Project_ASCs/4_Outputs/CSVs/6.1_Thirdstops_LEGs.csv"
data_LEGs = read.csv(source_LEGs, header = TRUE)

#Create dataframes for plot

md_total_1 <- median(data_total[["Pos1_difference"]])
md_total_2 <- median(data_total[["Pos2_difference"]])
md_total_3 <- median(data_total[["Pos3_difference"]])
md_total_4 <- median(data_total[["Pos4_difference"]])
md_total_5 <- median(data_total[["Pos5_difference"]])

md_HEGs_1 <- median(data_HEGs[["Pos1_difference"]])
md_HEGs_2 <- median(data_HEGs[["Pos2_difference"]])
md_HEGs_3 <- median(data_HEGs[["Pos3_difference"]])
md_HEGs_4 <- median(data_HEGs[["Pos4_difference"]])
md_HEGs_5 <- median(data_HEGs[["Pos5_difference"]])

md_LEGs_1 <- median(data_LEGs[["Pos1_difference"]])
md_LEGs_2 <- median(data_LEGs[["Pos2_difference"]])
md_LEGs_3 <- median(data_LEGs[["Pos3_difference"]])
md_LEGs_4 <- median(data_LEGs[["Pos4_difference"]])
md_LEGs_5 <- median(data_LEGs[["Pos5_difference"]])
...
par(mfrow=c(1,3), cex=1.05, cex.axis = 1.5)

barplot(total, names.arg = c("+1","+2","+3","+4","+5"), xlab="Position", ylab="Median difference (ASC-absent - ASC-containing)", col = "#01579B", ylim = c(-0.020, 0.03), main="All genes", cex.main=2, cex.lab=1.5)

barplot(HEGs, names.arg = c("+1","+2","+3","+4","+5"), xlab="Position", ylab="Median difference (ASC-absent - ASC-containing)", col = "#01579B", ylim = c(-0.02, 0.03), main="HEGs", cex.main=2, cex.lab=1.5)

barplot(LEGs, names.arg = c("+1","+2","+3","+4","+5"), xlab="Position", ylab="Median difference (ASC-absent - ASC-containing)", col = "#01579B", ylim = c(-0.02, 0.03), main="LEGs", cex.main=2, cex.lab=1.5)
...
w1 <- wilcox.test((data_HEGs[['Pos1_No_Stop']] - data_HEGs[['Pos1_Stop']]), (data_LEGs[['Pos1_No_Stop']] - data_LEGs[['Pos1_Stop']]))
w2 <- wilcox.test((data_HEGs[['Pos2_No_Stop']] - data_HEGs[['Pos2_Stop']]), (data_LEGs[['Pos2_No_Stop']] - data_LEGs[['Pos2_Stop']]))
w3 <- wilcox.test((data_HEGs[['Pos3_No_Stop']] - data_HEGs[['Pos3_Stop']]), (data_LEGs[['Pos3_No_Stop']] - data_LEGs[['Pos3_Stop']]))
w4 <- wilcox.test((data_HEGs[['Pos4_No_Stop']] - data_HEGs[['Pos4_Stop']]), (data_LEGs[['Pos4_No_Stop']] - data_LEGs[['Pos4_Stop']]))
w5 <- wilcox.test((data_HEGs[['Pos5_No_Stop']] - data_HEGs[['Pos5_Stop']]), (data_LEGs[['Pos5_No_Stop']] - data_LEGs[['Pos5_Stop']]))
