...
def main(source):
    ...
    for root, dirs, filenames in os.walk(source):
        ...
        for f in filenames:
            ...
            if len(pos1_add) > 0 and len(pos2_add) > 0 and len(pos3_add) > 0 and len(pos4_add) > 0 and len(pos5_add) > 0:
                if len(pos1_no_add) > 0 and len(pos2_no_add) > 0 and len(pos3_no_add) > 0 and len(pos4_no_add) > 0 and len(pos5_no_add) > 0:
...
def get_third_stop_frequencies(additional_list, no_additional_list, position):
    ...
    for i in additional_list:
        if 'taa' in i[position+1:len(i)-3] or 'tga' in i[position+1:len(i)-3] or 'tag' in i[position+1:len(i)-3]:
            add_counter += 1
...
def create_csv(csv_total, headers):
    ...
    filename = "6.3_Thirdstops_all.csv"
    #filename = "6.3_Thirdstops_HEGs.csv"
    #filename = "6.3_Thirdstops_LEGs.csv"
