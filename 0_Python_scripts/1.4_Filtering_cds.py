...
def get_utr_fasta(final_gene_samples, raw, sequence, CDS_cleaned, accession, GC, GC3):
    ...
    for t in range(1, w):
        ...
        if locus_tag in final_gene_samples:
            ...
            #Check cds sequences are suitable for analysis
            if cds == None:
                continue
            elif cds_start_check(cds) == False:
                continue
            elif nucleotide_check(cds) == False:
                continue
            elif length_check(cds) == False:
                continue
            elif cds_stop_check(cds) == False:
                continue
            else:
                fasta += FASTA_section
...
def FASTA_creation(fasta, accession):
    ...
    subdir = '2_FASTA_CDS_all'
    #subdir = '2_FASTA_CDS_paxdb'
...
def cds_start_check(sequence):
    s = len(sequence)
    if sequence[0:3] == 'atg':
        start = True
    elif sequence[0:3] == 'ttg':
        start = True
    elif sequence[0:3] == 'gtg':
        start = True
    else:
        start = False
    return start
